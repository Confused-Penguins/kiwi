...
class StorageService {
  ...
  Future<void> init() async {
    ...
    // 3. Auto-sync existing stored phone threat logs to live SOC Dashboard
    syncStoredLogsToSOC();
  }
  ...
  Future<void> logThreat(ThreatLogEntry entry) async {
    ...
    // Forward threat asynchronously to Kiwi SOC Telemetry Hub (non-blocking)
    _forwardIncidentToSOC(entry);
  }
  ...
  /// Asynchronously post incident report to SOC Web Portal backend
  Future<void> _forwardIncidentToSOC(ThreatLogEntry entry) async {
    final body = jsonEncode(entry.toJson());
    const headers = {"Content-Type": "application/json"};

    // 1. Try local Wi-Fi LAN IP (for physical mobile phones like I2403)
    try {
      final res = await http
          .post(Uri.parse(kSocTelemetryHubUrlLAN), headers: headers, body: body)
          .timeout(const Duration(seconds: 2));
      if (res.statusCode == 200 || res.statusCode == 201) return;
    } catch (_) {}

    // 2. Try localhost (for desktop/web/simulator)
    try {
      final res = await http
          .post(Uri.parse(kSocTelemetryHubUrl), headers: headers, body: body)
          .timeout(const Duration(seconds: 2));
      if (res.statusCode == 200 || res.statusCode == 201) return;
    } catch (_) {}

    // 3. Try Android emulator gateway (10.0.2.2)
    try {
      await http
          .post(Uri.parse(kSocTelemetryHubUrlAndroid), headers: headers, body: body)
          .timeout(const Duration(seconds: 2));
    } catch (_) {
      // Non-blocking fallback if SOC backend is offline
    }
  }

  /// Syncs all existing stored threat logs in the phone app to the SOC Telemetry Hub
  Future<void> syncStoredLogsToSOC() async {
    final logs = getThreatLogs();
    for (final entry in logs) {
      await _forwardIncidentToSOC(entry);
    }
  }
